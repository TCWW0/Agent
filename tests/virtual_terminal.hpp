...
public:
    ...
    // 终端尺寸变了。保留式语义（左上锚定）：旧格原样留下，加宽出的新列空白，
    // 变窄时右列被截断。依据：本项目驱动关 DECAWM（?7l），xterm 家族此时 resize
    // 不重排、只截断/补白 —— 这是对真实行为的忠实建模。更重要的身份是「最保守
    // 下界」：resize 后驱动对屏上有什么**一无所知**（除了这些还留着的旧格），
    // 所以它必须全量重绘才能重新建立 front == 终端像素；量具若把旧格清掉，
    // 「脏屏收敛」型断言就会空转（caret_seam 当年新建空白 VT 抓不到幽灵，
    // 同一个坑）。光标钳进新边界，待换行状态作废（旧右边距不再存在）。
